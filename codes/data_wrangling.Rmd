...
```{r}
load_raw <- read_excel("TSA_Competition/data/load.xlsx")
load <- load_raw %>%
...
humidity_raw <- read_excel("TSA_Competition/data/relative_humidity.xlsx")
...
temperature_raw <- read_excel("TSA_Competition/data/temperature.xlsx")
...
```{r}
NN_fit5 <- nnetar(ts_load_training,p=0,P=0,xreg=fourier(ts_load_training, K=c(2,12)))
NN_for5 <- forecast(NN_fit5,h=30, xreg=fourier(ts_load_training, K=c(2,12),h=30))
autoplot(NN_for5)
autoplot(ts_load_testing) +
  autolayer(NN_for5, series="Neural Network",PI=FALSE)
...
SS_fit1 <- StructTS(ts_load_training, type="BSM",fixed=c(NA,NA,NA,NA)) 
```
...
July_NN4
write.csv(July_NN1, file = "July_NN4.csv", row.names = FALSE)
```
